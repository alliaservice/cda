...
Optional[pd.DataFrame], Optional[pd.DataFrame]]:
    ...
    """
    print('data load abs path',abs_pth)
    portland = test_file(f"{term}_portland.xlsx", abs_pth)
    print('portland df len after load: ', len(portland))
    ds = test_file(f"{term}_full_ds.xlsx", abs_pth)
    print('ds df len after load: ', len(ds))
    #cda = test_file(f"{term}_output.xlsx", abs_pth)
    return portland, ds, #cda

def get_excel_file(prompt:str, abs_pth: str) -> pd.DataFrame:
    '''
...
def main():
    ...
    # Load data
    portland, ds = load_dataframes(term, abs_pth) # removed cda
    if any(df is None for df in [portland, ds]): # removed cda
    ...
    print('after data load, lengths are: \n Portland: ', len(portland), '\n Duck Store: ', len(ds)) # removed cda
    ...
    try:
        ...
        print('length of portland strip after CRN dedupe: ', len(portland_strip))
    except KeyError as e: # error handling for missing/renamed cols
